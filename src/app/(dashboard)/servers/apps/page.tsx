  ...
  const fetchApps = useCallback(async () => {
    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_MODE === "mock_data") {
        ...
        // Fetch from API
        const response = await fetch(`${urlApiForTsmNetwork}/pm2/apps`, {
          method: "GET",
          headers: {
    ...
    try {
      if (process.env.NEXT_PUBLIC_MODE === "mock_data") {
        ...
        const response = await fetch(
          `${urlApiForTsmNetwork}/pm2/toggle-app-status/${appToToggle.name}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          showInfoModal(
            "Status Changed",
            resJson.message || "App status toggled successfully",
            "success"
          );
        } else {
          const errorMessage =
