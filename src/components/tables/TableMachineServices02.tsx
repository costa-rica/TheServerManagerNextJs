	...
	const columns: ColumnDef<Service>[] = [
					...
					<div className="flex items-center gap-3">
						...
						<div className="flex-1 min-w-0">
							<div className="flex items-center gap-2">
								...
								{hasTimer && (
									<TimeIcon className="w-3.5 h-3.5 md:w-4 md:h-4 text-info-500 flex-shrink-0" title="Has timer" />
								)}
							</div>
				...
				return (
					<div className="hidden md:flex items-center gap-2">
						...
						<button
							...
							disabled={isToggleLoading(service.name, isRunning ? "stop" : "start")}
						...
						<button
							...
							disabled={isToggleLoading(service.name, isEnabled ? "disable" : "enable")}
							className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
								isEnabled
									? "bg-success-100 dark:bg-success-900/30 text-success-700 dark:text-success-400 hover:bg-success-200 dark:hover:bg-success-900/50"
		...
		},
		{
			...
			cell: ({ row }) => {
				const service = row.original;
				if (!service.timerStatus) {
					return <span className="hidden lg:block text-xs text-gray-400 dark:text-gray-600">-</span>;
				}
				...
				return (
					<div className="hidden lg:flex items-center gap-2">
						...
						<button
							...
							disabled={isToggleLoading(service.name, `timer-${timerRunning ? "stop" : "start"}`)}
							className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
								timerRunning
									? "bg-error-100 dark:bg-error-900/30 text-error-700 dark:text-error-400 hover:bg-error-200 dark:hover:bg-error-900/50"
						...
						<button
							...
							disabled={isToggleLoading(service.name, `timer-${timerEnabled ? "disable" : "enable"}`)}
							className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
								timerEnabled
									? "bg-success-100 dark:bg-success-900/30 text-success-700 dark:text-success-400 hover:bg-success-200 dark:hover:bg-success-900/50"
									: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"
														...
														<div className="flex gap-2">
															...
															<button
																...
																onClick={() =>
																	handleToggle(
																		service.filename,
																		service.onStartStatus === "enabled" ? "disable" : "enable",
																		service.name
																	)
																}
																disabled={isToggleLoading(
															...
															>
																...
																{service.onStartStatus === "enabled" ? "Enabled" : "Disabled"}
															</button>
														</div>
													</div>
															...
															<div className="space-y-2">
																...
																<div className="flex gap-2">
																	<button
																		type="button"
																		onClick={() => {
																			const timerFilename = service.filename.replace(
																				".service",
																				".timer"
																			);
																			handleToggle(
																	...
																	<button
																		...
																		disabled={isToggleLoading(
																			service.name,
																			`timer-${service.timerOnStartStatus === "enabled" ? "disable" : "enable"}`
																		)}
																		className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-50 ${
																			service.timerOnStartStatus === "enabled"
																				? "bg-success-100 dark:bg-success-900/30 text-success-700 dark:text-success-400"
																	...
																	>
																		...
																		{service.timerOnStartStatus === "enabled" ? "Enabled" : "Disabled"}
																	</button>
																</div>
															</div>
														</div>
