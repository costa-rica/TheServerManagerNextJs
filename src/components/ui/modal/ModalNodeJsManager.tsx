	...
	const handleNpmAction = async (action: "install" | "build") => {
		...
		try {
			...
			const response = await fetch(
				`${connectedMachine.urlFor404Api}/services/npm/${encodedServiceName}/${action}`,
				{
					method: "POST",
					headers: {
			...
			if (onError) {
				onError({
					code: "NETWORK_ERROR",
					message: err instanceof Error ? err.message : `Failed to run npm ${action}`,
				...
				<div className="space-y-6">
					...
					<div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
						...
						<p className="text-xs text-gray-500 dark:text-gray-400 mt-3">
							<strong>Install:</strong> Runs npm install to install dependencies.{" "}
							<strong>Build:</strong> Runs npm run build to build the project.
						</p>
					</div>
