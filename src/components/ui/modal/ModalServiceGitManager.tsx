...
import { ChevronDownIcon, ChevronUpIcon, CheckCircleIcon, TrashBinIcon } from "@/icons";
...
export const ModalServiceGitManager: React.FC<ModalServiceGitManagerProps> = ({
	...
	const fetchBranches = async () => {
		...
		try {
			...
			const response = await fetch(
				`${connectedMachine.urlFor404Api}/services/git/${encodedServiceName}`,
				{
					method: "GET",
					headers: {
	...
	const handleGitAction = async (action: "fetch" | "pull") => {
		...
		try {
			const encodedServiceName = encodeURIComponent(serviceName);
			const response = await fetch(
				`${connectedMachine.urlFor404Api}/services/git/${encodedServiceName}/${action}`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const handleCheckoutBranch = async (branchName: string) => {
		...
		showLoading({ message: `Switching to branch ${branchName}...`, variant: "info" });

		try {
			const encodedServiceName = encodeURIComponent(serviceName);
			const encodedBranchName = encodeURIComponent(branchName);
			const response = await fetch(
				`${connectedMachine.urlFor404Api}/services/git/checkout/${encodedServiceName}/${encodedBranchName}`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
			...
			if (onError) {
				onError({
					code: "NETWORK_ERROR",
					message: err instanceof Error ? err.message : `Failed to checkout branch ${branchName}`,
					details: "Unable to connect to the server",
					status: 0,
				});
			}
	...
	const handleDeleteBranch = async (branchName: string) => {
		...
		showLoading({ message: `Deleting branch ${branchName}...`, variant: "warning" });

		try {
			const encodedServiceName = encodeURIComponent(serviceName);
			const encodedBranchName = encodeURIComponent(branchName);
			const response = await fetch(
				`${connectedMachine.urlFor404Api}/services/git/delete-branch/${encodedServiceName}/${encodedBranchName}`,
				{
					method: "DELETE",
					headers: {
						Authorization: `Bearer ${token}`,
			...
			if (onError) {
				onError({
					code: "NETWORK_ERROR",
					message: err instanceof Error ? err.message : `Failed to delete branch ${branchName}`,
					details: "Unable to connect to the server",
					status: 0,
				});
			}
